...
pytestmark = pytest.mark.filterwarnings("ignore:MCPTool class is deprecated:DeprecationWarning")
...
google.adk.tools.StreamableHTTPConnectionParams = getattr(google.adk.tools, "StreamableHTTPConnectionParams", DummyStreamableHTTPConnectionParams)
...
with patch("google.adk.integrations.agent_registry.agent_registry.AgentRegistry.get_mcp_server", mock_get_mcp_server), \
     patch("google.adk.integrations.agent_registry.agent_registry.AgentRegistry.get_mcp_toolset", lambda self, name: search_logs):
...
async def test_mcp_logging_call(monkeypatch):
    ...
    monkeypatch.setattr("google.adk.tools.mcp_tool.mcp_session_manager.MCPSessionManager.create_session", mock_create_session)
    ...
    events = [
        ...
        MockEvent("LoggingAgent", types.Content(role="user", parts=[types.Part(function_response=types.FunctionResponse(name="search_logs", response={"result": "found 0 errors"}))])),
        MockEvent("LoggingAgent", types.Content(role="model", parts=[types.Part(text="I checked the logs and found 0 errors.")]))
    ]
...
@pytest.mark.skipif(not hasattr(o11y_agent_module, "trace_agent"), reason="Trace agent not available")
@pytest.mark.asyncio
async def test_mcp_trace_call(monkeypatch):
    ...
    mock_session.call_tool.return_value = CallToolResult(
        content=[TextContent(type="text", text="Trace search results: found high latency spans")]
    )
    ...
    message = types.Content(
        role="user", parts=[types.Part.from_text(text="Use the search_traces tool to check traces for latency greater than 1s.")]
    )
...
@pytest.mark.skipif(not hasattr(o11y_agent_module, "metrics_agent"), reason="Metrics agent not available")
@pytest.mark.asyncio
async def test_mcp_metrics_call(monkeypatch):
    ...
    mock_session.call_tool.return_value = CallToolResult(
        content=[TextContent(type="text", text="Metrics results: CPU utilization is 45%")]
    )
    ...
    session_service = InMemorySessionService()
    session = session_service.create_session_sync(user_id="test_user", app_name="test")
    runner = Runner(agent=metrics_agent, session_service=session_service, app_name="test")

    message = types.Content(
        role="user", parts=[types.Part.from_text(text="Use the list_time_series tool to check CPU utilization metrics.")]
    )
